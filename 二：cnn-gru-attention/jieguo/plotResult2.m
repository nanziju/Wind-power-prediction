...
kelm     = xlsread('LSTM2.xlsx');
lstm            =xlsread('CNN.xlsx');
 libsvm      = xlsread('GRU.xlsx');
 cnn1    =xlsread('bilstm2.xlsx');
 cnn2    =xlsread('cnn-bilstm2.xlsx');
 cnn    =xlsread('cnn-gru2.xlsx');
prophet     =xlsread('CNN-GRU-Attention.xlsx');
...
plot(cnn1,'color',pColor(5,:),'linewidth', linewidth);
plot(cnn2,'color',pColor(6,:),'linewidth', linewidth);
...
ylabel('Wind wind power(MW)','FontName','Times New Roman','FontAngle','italic');
axis tight;
set(gca,'FontName','Times New Roman','FontSize',fontSize,'Box','on','TickDir','in','linewidth', linewidth);
legend('Actual Wind Power','LSTM','CNN','GRU','BILSTM','CNN-BILSTM','CNN-GRU','CNN-GRU-Attention','FontName','Times New Roman','FontSize',10,'FontAngle','italic','Box','off');
...
scatter(orignal, libsvm, 'MarkerEdgeColor', pColor(4,:),'linewidth', linewidth);
scatter(orignal, cnn1, 'MarkerEdgeColor', pColor(5,:),'linewidth', linewidth);
scatter(orignal, cnn2, 'MarkerEdgeColor', pColor(6,:),'linewidth', linewidth);
...
mse_prophet_arima1 =  mean((orignal-cnn1).^2);
mse_neuralprophet1 =  mean((orignal-cnn2).^2);
...
disp(['mse = ', num2str(mse_prophet_arima1)])
disp(['mse = ', num2str(mse_neuralprophet1)])
...
mae_prophet_arima1 = sum(abs(orignal-cnn1))/length(orignal);
mae_neuralprophet1 = sum(abs(orignal-cnn2))/length(orignal);
...
disp(['mae = ', num2str(mae_prophet_arima1)])
disp(['mae = ', num2str(mae_neuralprophet1 )])
...
rmse_prophet_arima1 = sqrt(sum(abs(orignal-cnn1).^2)/length(orignal));
rmse_neuralprophet1 = sqrt(sum(abs(orignal-cnn2).^2)/length(orignal));
...
numerator2=sqrt(sum((orignal - cnn1).^2));
denominator2=sqrt(sum(orignal.^2)) + sqrt(sum(cnn1.^2));
R2_prophet_arima1 =numerator2 / denominator2; 

numerator3=sqrt(sum((orignal - cnn2).^2));
denominator3 = sqrt(sum(orignal.^2)) + sqrt(sum(cnn2.^2))
R2_neuralprophet1 = numerator3 / denominator3 ;
...
R2_prophet_arima11= sqrt(sum((orignal - cnn1).^2) / n) / (sqrt(sum((orignal- mean(orignal)).^2) / n) + sqrt(sum((cnn1 - mean(cnn1)).^2) / n));
R2_neuralprophet11= sqrt(sum((orignal - cnn2).^2) / n) / (sqrt(sum((orignal- mean(orignal)).^2) / n) + sqrt(sum((cnn2 - mean(cnn2)).^2) / n));
...
mape_neuralprophet = sum(abs((orignal-libsvm)./orignal))/length(orignal);
 mape_prophet_arima1 = sum(abs((orignal-cnn1)./orignal))/length(orignal);
 mape_neuralprophet1 = sum(abs((orignal-cnn2)./orignal))/length(orignal);
...
disp(['mape = ', num2str(mape_neuralprophet)])
 disp(['mape = ', num2str(mape_prophet_arima1)])
 disp(['mape = ', num2str(mape_neuralprophet1)])
...
for i = 1:n
    if orignal(i) > cnn1(i)
        pinball_loss = pinball_loss + tau * (orignal(i) - cnn1(i));
    else
        pinball_loss = pinball_loss + (1 - tau) * (cnn1(i) - orignal(i));
    end
end
% 计算平均弹球损失
p4 = pinball_loss / n;
...
for i = 1:n
    if orignal(i) > cnn2(i)
        pinball_loss = pinball_loss + tau * (orignal(i) - cnn2(i));
    else
        pinball_loss = pinball_loss + (1 - tau) * (cnn2(i) - orignal(i));
    end
end
% 计算平均弹球损失
p5 = pinball_loss / n;
...
disp(['rmse = ', num2str(rmse_prophet_arima1)])
disp(['rmse = ', num2str(rmse_neuralprophet1)])
...
disp(['Theil1s U统计量 = ', num2str(R2_prophet_arima1)])
disp(['Theil1s U统计量 = ', num2str(R2_neuralprophet1)])
...
disp([' TIC  = ', num2str(R2_prophet_arima11)])
disp([' TIC  = ', num2str(R2_neuralprophet11)])
...
% 
% b_mae_prophet_arima = bar(4,mae_prophet_arima1);
% set(b_mae_prophet_arima, 'FaceColor', bColor(4,:));
% b_mae_neuralprophet = bar(5,mae_neuralprophet1);
% set(b_mae_neuralprophet, 'FaceColor', bColor(5,:));
% 
% b_mae_neuralprophet_arima = bar(6,mae_neuralprophet_arima);
% set(b_mae_neuralprophet_arima, 'FaceColor', bColor(6,:));
% b_mae_mlp = bar(7,mae_mlp);
% set(b_mae_mlp, 'FaceColor', bColor(7,:));
% 
% xlabel('Date 1','FontSize',fontSize, 'FontName', 'Times New Roman', 'FontWeight', 'normal', 'FontAngle', 'italic');
...
xlabel('Date 2','FontSize',fontSize, 'FontName', 'Times New Roman', 'FontWeight', 'normal', 'FontAngle', 'italic');
ylabel('MSE','FontSize',fontSize, 'FontName', 'Times New Roman', 'FontWeight', 'normal', 'FontAngle', 'italic');
set(gca,'XTickLabel',[]);
set(gca,'FontName','Times New Roman','FontSize',fontSize,'Box','on','TickDir','in','linewidth', linewidth);

legend('LSTM','CNN','GRU','BILSTM','CNN-BILSTM','CNN-GRU','CNN-GRU-Attention','FontName','Times New Roman','FontSize',10,'FontAngle','italic','Box','off','Location','northwest');
...
b_rmse_prophet_arima = bar(4,rmse_prophet_arima1);
set(b_rmse_prophet_arima, 'FaceColor', bColor(4,:));
b_rmse_neuralprophet = bar(5,rmse_neuralprophet1);
...
b_mape_neuralprophet1 = bar(3,mape_neuralprophet);
set(b_mape_neuralprophet1, 'FaceColor', bColor(3,:));
b_mape_prophet_arima1 = bar(4,mape_prophet_arima1);
set(b_mape_prophet_arima1, 'FaceColor', bColor(4,:));
b_mape_neuralprophet = bar(5,mape_neuralprophet1);
...
subplot(2,6,11);
hold on;
b_R2_prophet2 = bar(1,R2_prophet1);
set(b_R2_prophet2, 'FaceColor', bColor(1,:));
b_R2_prophet_arima2 = bar(2,R2_prophet_arima1);
set(b_R2_prophet_arima2, 'FaceColor', bColor(2,:));
b_R2_neuralprophet2 = bar(3,R2_neuralprophet1);
set(b_R2_neuralprophet2, 'FaceColor', bColor(3,:));

b_R2_prophet_arima2 = bar(4,R2_prophet_arima11);
set(b_R2_prophet_arima2, 'FaceColor', bColor(4,:));
b_R2_neuralprophet2 = bar(5,R2_neuralprophet11);
set(b_R2_neuralprophet2, 'FaceColor', bColor(5,:));

b_R2_neuralprophet_arima2 = bar(6,R2_neuralprophet_arima1);
set(b_R2_neuralprophet_arima2, 'FaceColor', bColor(6,:));
b_R2_mlp2 = bar(7,R2_mlp1);
